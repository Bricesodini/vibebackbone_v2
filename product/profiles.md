...
## Bilan rc.6 et candidat de contrat rc.7

La campagne rc.6 a collecté dix cellules métier baseline/VBB, avec revue et remédiation natives préfixées, deux enfants DSH réels et des lecteurs neufs sur les trois routes. Les corrections métier passent leurs assertions ciblées ; la fidélité n'est pas validée : fausses absences, omissions de restitution et confusion entre preuve et résumé demeurent. Les réparations natives préfixées ne sont pas une réussite initiale autonome. Aucun lecteur échoué n'est remplacé dans le bilan.

Un diagnostic distinct sur douze lectures de documents inchangés observe davantage de restitution avec une demande explicite de couverture, mais encore des erreurs et davantage de texte/temps. Il ne constitue ni confirmation nouvelle ni preuve d'utilité d'un stockage supplémentaire. Rc.7 précise le contrat de restitution, la filiation des notes, les assertions d'absence et la portée des preuves ; cette correction de conduite reste à qualifier sur de nouveaux cas. Les deux moteurs et le cycle de vie ne changent pas.

Les versions du préparateur ne suffisent pas à identifier celles des commandes natives. Les tests navigateur G1 ont utilisé Node20.19.0 sous Codex et Node24.14.1 sous Pi/DSH. Lors des smokes de mesures rc.6, DSH a relevé Python3.9.6, hors du contrat3.10+ ; les mesures ciblées ont fonctionné, sans qualifier toutes fonctions sous3.9. Pi a appelé directement continuity.py, sans prouver le chemin public vbb.py dans ce passage. Python effectif Codex/Pi non mesuré dans ces smokes. Le prochain gel doit constater le chemin et la version de l'interpréteur supporté et qualifier le point d'entrée public, sans réétiqueter les passages précédents.
